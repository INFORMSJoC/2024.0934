...
public class BranchAndBound_CBFS {
    double lowerBound;
    double upperBound;
    treeBB incumbent_node;
    public double terminateTime = 7200;
    ...
    private final ConcurrentHashMap<Integer, PriorityQueue<treeBB>> depthQueues;
    ...
    public BranchAndBound_CBFS() {
        this.lowerBound = -1e10;
        this.upperBound = 1e10;
        this.executorService = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());
        this.depthQueues = new ConcurrentHashMap<>();
    ...
    class NodeProcessor implements Runnable {
        ...
        public void run() {
            try {
                while (!isTerminated.get()) {
                    treeBB currentNode = null;
                    synchronized (queueLock) {
                        // Get the next depth to process
                        int depth = currentDepth.get();
                        PriorityQueue<treeBB> queue = depthQueues.get(depth);
                        int nextDepth = findNextNonEmptyDepth(depth);  // find the next non-empty depth
                        if (nextDepth == -1) {
        ...
        private int findNextNonEmptyDepth(int currentDepth) {
            for (int depth : depthQueues.keySet()) {
                if (depth > currentDepth && !depthQueues.get(depth).isEmpty()) {
                    return depth;
                }
            }
            // If no deeper depth found, start from the beginning
            for (int depth : depthQueues.keySet()) {
                if (!depthQueues.get(depth).isEmpty()) {
                    return depth;
                }
            }
            return -1;
...
//                            System.out.println("CUT | BB Depth: " + currentNode.depth + " | Local CG cost: " + currentNode.localLowerBound);
        ...
        private void addNodeToDepthQueue(treeBB node) {
            synchronized (queueLock) {
                depthQueues.computeIfAbsent(node.depth, k -> 
                    new PriorityQueue<>((a, b) -> Double.compare(a.localLowerBound, b.localLowerBound))
                ).offer(node);
            }
        }
        ...
        private treeBB createRejectionNode(treeBB currentNode, int[] assignedPair, Instance instance) {
            ...
            rejNode.localLowerBound = currentNode.localLowerBound; // Initialize the lower bound to the parent node's value
        ...
        private treeBB createAssignmentNode(treeBB currentNode, int[] assignedPair, int truckIndex, Instance instance) {
            ...
            newNode.localLowerBound = currentNode.localLowerBound; // Initialize the lower bound to the parent node's value
    ...
    public void BBnode(Instance instance, ArrayList<Route> initRoutes) throws IOException {
        double eps = 1e-3;
        ...
        // Add root node to depth queue
        synchronized (queueLock) {
            depthQueues.computeIfAbsent(0, k -> 
                new PriorityQueue<>((a, b) -> Double.compare(a.localLowerBound, b.localLowerBound))
            ).offer(initialNode);
        }
        activeTasks.incrementAndGet();
        ...
        int cnt = 0;
        try {
            while (!(activeTasks.get() == 0) &&  // Main termination condition: no active threads
                ...
                if (cnt % 1000 == 0) {
                    System.out.println("Time:" + (System.currentTimeMillis() - startTime) / 1000 + "ACTIVE TASKS: " + activeTasks.get() + " | Total BBnode: " + branchNum);
                }
            }
