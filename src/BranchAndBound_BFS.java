...
public class BranchAndBound_BFS {
    double lowerBound;
    double upperBound;
    ...
    private final PriorityQueue<treeBB> priorityQueue;
    ...
    private final Object queueLock = new Object(); // Lock for priority queue operations

    public BranchAndBound_BFS() {
        this.lowerBound = -1e10;
        this.upperBound = 1e10;
        this.executorService = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());
        this.priorityQueue = new PriorityQueue<>((a, b) -> 
            Double.compare(a.localLowerBound, b.localLowerBound)
        );
        ...
        public void run() {
            try {
                while (!isTerminated.get()) {
                    ...
                    if (currentNode == null) {
                        if (activeTasks.get() == 0 && priorityQueue.isEmpty()) {
                            break;
                        }
...
//                            System.out.println("CUT | BB Depth: " + currentNode.depth + " | Local CG cost: " + currentNode.localLowerBound);
    ...
    public void BBnode(Instance instance, ArrayList<Route> initRoutes) throws IOException {
        ...
        synchronized (queueLock) {
            priorityQueue.offer(initialNode);
        }
        activeTasks.incrementAndGet();
        ...
        int cnt = 0;
        try {
            while (!(activeTasks.get() == 0 && priorityQueue.size() == 0) &&  // Main termination condition: no active threads and the queue is empty
                ...
                if (cnt % 1000 == 0) {
                    System.out.println("ACTIVE TASKS: " + activeTasks.get() + " queue size: " + priorityQueue.size());
                }
            }
